...
# باز کردن مرورگر با چک اتصال
driver = webdriver.Chrome(service=service, options=chrome_options)
print("کروم باز شد!")
for attempt in range(3):  # 3 بار تلاش برای اتصال
    try:
    ...
    print("اتصال به تلگرام وب ممکن نشد. اینترنتت رو چک کن!")
    driver.quit()
    exit()

time.sleep(30)  # 30 ثانیه برای لاگین

# رفتن به کانال با چک اتصال
for attempt in range(3):
    try:
        ...
        print(f"خطا در رفتن به کانال (تلاش {attempt + 1}): {e}")
        time.sleep(5)
else:
    print("نشد به کانال وصل بشیم. اینترنتت رو چک کن!")
    driver.quit()
    exit()

time.sleep(20)  # صبر برای لود کامل

# موقعیت موس رو به وسط صفحه می‌برم
pyautogui.moveTo(960, 540)  # رزولوشن 1920x1080 فرض شده

# اسکرول به بالا با PyAutoGUI
print("در حال اسکرول به بالای کانال...")
for _ in range(20):  # 20 بار اسکرول
    pyautogui.scroll(500)  # 500 واحد به بالا
