...
# اسکرول تدریجی به بالا
print("در حال اسکرول به بالای کانال...")
scroll_script = """
    var bubbles = document.querySelector('.bubbles-inner');
    var scrollStep = -500; // هر بار 500 پیکسل به بالا
    var currentScroll = bubbles.scrollTop;
    function smoothScroll() {
        if (currentScroll > 0) {
            currentScroll += scrollStep;
            if (currentScroll < 0) currentScroll = 0;
            bubbles.scrollTo(0, currentScroll);
            setTimeout(smoothScroll, 100); // 100 میلی‌ثانیه صبر
        }
    }
    smoothScroll();
    return bubbles.scrollTop;
"""
while True:
    current_scroll = driver.execute_script(scroll_script)
    print(f"مقدار scrollTop: {current_scroll}")
    time.sleep(2)  # صبر برای لود پست‌ها
    if current_scroll == 0:
